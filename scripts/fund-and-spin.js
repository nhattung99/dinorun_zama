/*
  Fund races then race (test flow):

  Steps:
    1) Try dailyGm() to add +1 race
    2) If still no race, buy GM with ETH (0.02 ETH) then buyRaceWithGmBatch(1)
    3) Call race() and report result (or HCU-likely revert)

  Usage:
    node scripts/fund-and-race.js <contractAddress>
...
async function main() {
  ...
  if (!pk || !rpc || !addrArg || !ethers.isAddress(addrArg)) {
    console.error(
      "Usage: node scripts/fund-and-race.js <contractAddress>  (requires REACT_APP_PRIVATE_KEY, REACT_APP_SEPOLIA_RPC_URL)",
    );
    process.exit(1);
  }
  ...
  // 2) Buy GM => Buy Race (fallback path)
  try {
  ...
  try {
    console.log("➡️  buyRaceWithGmBatch(1) ...");
    const rc = await withOverrides("buyRaceWithGmBatch", [1]);
    console.log("buyRaceWithGmBatch status:", rc.status);
  } catch (e) {
    console.warn("buyRaceWithGmBatch failed, trying single:", e?.shortMessage || e?.reason || e?.message || String(e));
    try {
      const rc2 = await withOverrides("buyRaceWithGm", []);
      console.log("buyRaceWithGm status:", rc2.status);
    } catch (e2) {
      console.warn("buyRaceWithGm failed:", e2?.shortMessage || e2?.reason || e2?.message || String(e2));
    }
  }
  ...
  try {
    ...
    try {
      const est = await c.race.estimateGas();
      const cap = 3_500_000n;
      overrides.gasLimit = est * 2n > cap ? cap : est * 2n;
    } catch {
      overrides.gasLimit = 3_000_000n;
    }
    const tx = await c.race(overrides);
    const rc = await tx.wait();
    console.log("race status:", rc.status, "gasUsed:", rc.gasUsed?.toString?.());
    console.log("logs:", rc.logs?.length || 0);
  } catch (e) {
    if (isLikelyHcuLimit(e)) console.error("race: HCU_LIMIT_EXCEEDED (likely)");
    else console.error("race failed:", e?.shortMessage || e?.reason || e?.message || String(e));
  }
}
