/*
  Quick race HCU tester

  Usage:
    node scripts/test-race-hcu.js <contractAddress?>
...
dotenv.config({ path: "frontend-fhe-race/.env" });
dotenv.config({ path: "frontend-fhe-race/.env.local" });
  ...
  async function attemptRace() {
    ...
    try {
      ...
      if (isLikelyHcuLimit(e)) {
        ...
        console.error("race failed:", e?.shortMessage || e?.reason || e?.message || e);
      }
