...
async function main() {
  ...
  // 2) Read encrypted races
  const encryptedRaces: string = await contract.getUserRaces(user);
  console.log("encryptedRaces:", encryptedRaces);
  ...
  // 4) EIP-712 user-decrypt flow to decrypt races
  ...
  const pairs = [{ handle: encryptedRaces, contractAddress }];
  ...
  const decrypted = res[encryptedRaces];
  console.log("Decrypted races:", decrypted);
}
