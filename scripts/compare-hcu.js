/*
  Compare HCU behavior between two contract addresses by attempting race
...
async function attemptRace(contract, provider) {
  const overrides = {};
  try {
    const est = await contract.race.estimateGas();
  ...
  try {
    const tx = await contract.race(overrides);
...
async function testAddress(address, wallet, provider, rounds = 3) {
  ...
  for (let i = 0; i < rounds; i++) {
    ...
    const r = await attemptRace(c, provider);
    results.push(r);
    if (r.ok) break;
    // eslint-disable-next-line no-await-in-loop
