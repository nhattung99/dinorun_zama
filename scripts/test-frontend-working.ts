...
async function main() {
  console.log("🧪 Testing Frontend Working Status...");
  ...
  try {
    ...
    try {
      const racePurchasedEvents = await luckyRaceFHE.queryFilter(
        luckyRaceFHE.filters.RacePurchased(),
        fromBlock
      );
      console.log(`🛒 Race Purchased Events: ${racePurchasedEvents.length}`);

      const raceCompletedEvents = await luckyRaceFHE.queryFilter(
        luckyRaceFHE.filters.RaceCompleted(),
        fromBlock
      );
      console.log(`🎰 Race Completed Events: ${raceCompletedEvents.length}`);

      const gmTokensBoughtEvents = await luckyRaceFHE.queryFilter(
        luckyRaceFHE.filters.GmTokensBought(),
        fromBlock
      );
      console.log(`🪙 GM Tokens Bought Events: ${gmTokensBoughtEvents.length}`);

      const dailyGmCompletedEvents = await luckyRaceFHE.queryFilter(
        luckyRaceFHE.filters.DailyGmCompleted(),
        fromBlock
      );
    ...
    console.log("\n🎉 Frontend Working Test Completed Successfully!");
    ...
    console.error("❌ Error during frontend working test:", error);
  }
}

main()
  .then(() => {
    console.log("\n✅ Frontend working test completed!");
    process.exit(0);
  })
  .catch((error) => {
