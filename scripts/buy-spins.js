/*
  Buy races with GM on Strict contract

  Usage:
    node scripts/buy-races.js <contractAddress> [count]

  Notes:
    - GM rate = 1000 GM / 1 ETH; cost per race = 10 GM
...
async function main() {
  ...
  if (!pk || !rpc || !addrArg || !ethers.isAddress(addrArg)) {
    console.error("Usage: node scripts/buy-races.js <contractAddress> [count]");
    process.exit(1);
  }
  ...
  console.log(`Buying GM with ~${ethNeeded} ETH for ${count} race(s)`);
  ...
  // 2) Buy races using GM
  let rc2;
  try {
    const tx2 = await c.buyRaceWithGmBatch(count, overrides);
    rc2 = await tx2.wait();
    console.log("buyRaceWithGmBatch status:", rc2.status);
  } catch (e) {
    console.warn("buyRaceWithGmBatch failed, trying single calls ...");
    for (let i = 0; i < count; i++) {
      // eslint-disable-next-line no-await-in-loop
      const txSingle = await c.buyRaceWithGm(overrides);
      // eslint-disable-next-line no-await-in-loop
      const rcSingle = await txSingle.wait();
      console.log(`buyRaceWithGm #${i + 1} status:`, rcSingle.status);
      rc2 = rcSingle;
    }
  }
