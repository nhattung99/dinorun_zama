...
async function main() {
  ...
  // Fund ETH pool
  await luckyRaceFHE.fundPoolWithETH(0, { value: ethers.parseEther("2.0") });
  console.log("✅ Funded ETH pool with 2 ETH");

  // Check pool balance
  const ethPool = await luckyRaceFHE.getPoolReward(0);
  ...
  await luckyRaceFHE.updatePointsConfig(
    15, // checkInPoints
    8, // racePoints
    5, // gmPoints
    25, // winBonusPoints
  ...
  // Update race config
  await luckyRaceFHE.updateRaceConfig(
    2, // baseRacesPerCheckIn
    3, // bonusRacesPerGM
    15, // maxRacesPerDay
    4, // unluckySlotCount
  );
  console.log("✅ Updated race configuration");

  // Update unlucky slots
  await luckyRaceFHE.updateUnluckySlots([1, 3, 7, 9]);
  ...
  // Add NFT reward
  await luckyRaceFHE.addNFTReward(
    2, // NFT pool index
    12345, // tokenId
  ...
  // User1: Buy races (không refund)
  console.log("\n👤 User1 Actions:");
  await luckyRaceFHE.connect(user1).buyRaces(5, {
    value: ethers.parseEther("0.05"), // 5 races * 0.01 ETH
  });
  console.log("✅ User1 bought 5 races for 0.05 ETH (no refund)");

  // User1: Check-in
  await luckyRaceFHE.connect(user1).checkIn();
  console.log("✅ User1 checked in (received races + points)");

  // User1: Send GM
  await luckyRaceFHE.connect(user1).sendGM();
  console.log("✅ User1 sent GM (received bonus races + points)");

  // User2: Different actions
  console.log("\n👤 User2 Actions:");
  await luckyRaceFHE.connect(user2).buyRaces(3, {
    value: ethers.parseEther("0.03"),
  });
  console.log("✅ User2 bought 3 races for 0.03 ETH");

  await luckyRaceFHE.connect(user2).checkIn();
  ...
  console.log("✅ Configuration Management (Points & Race configs)");
  console.log("✅ NFT Rewards System");
  console.log("✅ User Actions (Buy races, Check-in, GM)");
