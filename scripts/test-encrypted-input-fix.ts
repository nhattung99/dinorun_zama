...
async function main() {
  ...
  try {
    ...
    try {
      const contractBalance = await luckyRaceFHE.getContractBalance();
    ...
    try {
      const canGm = await luckyRaceFHE.canGmToday(owner);
    ...
    try {
      const userRaces = await luckyRaceFHE.getUserRaces(owner);
      console.log("✅ User Races (encrypted):", userRaces);
    } catch (error) {
      console.log("⚠️ Could not get user races:", error.message);
    }
    
    try {
      const userRewards = await luckyRaceFHE.getUserRewards(owner);
