...
async function testDailyGmFeatures() {
  ...
  try {
    ...
    const contractABI = [
      ...
      "function userRaces(address user) external view returns (bytes)",
    ...
    // ✅ Test 5: Get user races (encrypted)
    console.log("\n🎰 Testing User Races...");
    const userRaces = await contract.userRaces(deployer.address);
    ...
    console.log("  - Last GM time:", lastGmTime.toString());
    console.log("  - Time until next GM:", timeUntilNextGm.toString(), "seconds");
    console.log("  - User races:", userRaces);
