/*
  ...
  Defaults:
    contractAddress defaults to the one in frontend-fhe-race/env.local if not provided,
...
async function main() {
  ...
  tryLoadEnvFile("frontend-fhe-race/.env");
  tryLoadEnvFile("frontend-fhe-race/.env.local");
  tryLoadEnvFile("frontend-fhe-race/env.local");
  ...
  const [raceLogs, buyRacesLogs, checkinLogs, gmBuyLogs] = await Promise.all([
    getLogsByTopic(RACE_OUTCOME_SIG),
    getLogsByTopic(RACE_BOUGHT_SIG),
  ...
  const racesDone = BigInt(Array.isArray(raceLogs) ? raceLogs.length : 0);
  ...
  const availableRaces = racesBought + checkins - racesDone;
  const gmEstimated = gmPublic + gmFromPrizes - 10n * racesBought; // cost: 10 GM per race
  ...
  const result = {
    contract,
    user,
    availableRaces: Number(availableRaces < 0n ? 0n : availableRaces),
    gmEstimated: Number(gmEstimated < 0n ? 0n : gmEstimated),
    pendingEth: formatEth(pendingEthWei),
    lastSlot: lastSlotNum,
    _debug: {
      racesDone: Number(racesDone),
      racesBought: Number(racesBought),
      checkins: Number(checkins),
      gmPublic: Number(gmPublic),
      gmFromPrizes: Number(gmFromPrizes),
      gmEstimatedFormula: "gmPublic + gmFromPrizes - 10 * racesBought",
      raceLogs: raceLogs.slice(-3).map((l) => ({ tx: l.transactionHash, block: l.blockNumber })),
      buyRacesLogs: buyRacesLogs.slice(-3).map((l) => ({ tx: l.transactionHash, block: l.blockNumber })),
