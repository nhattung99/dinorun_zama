/*
  Race test using raceLite() then settlePrize(slot)

  Usage:
    node scripts/race-lite-and-settle.js <contractAddress>
...
async function main() {
  ...
  if (!pk || !rpc || !addrArg || !ethers.isAddress(addrArg)) {
    console.error("Usage: node scripts/race-lite-and-settle.js <contractAddress>");
    process.exit(1);
  }
  ...
  try {
    const est = await c.raceLite.estimateGas();
  ...
  console.log("\n➡️  Calling raceLite() ...");
  const tx = await c.raceLite(overrides);
  const rc = await tx.wait();
  console.log("raceLite status:", rc.status, "logs:", rc.logs?.length || 0);

  const topic = ethers.id("RaceOutcome(address,uint8,uint256,uint64)");
  const log = rc.logs.find((l) => l.topics?.[0] === topic);
  if (!log) {
    console.log("No RaceOutcome log found.");
    return;
  }
