...
import { CONFIG } from "../frontend-fhe-race/src/config";
// Import ABI directly
...
async function testAppAbiIntegration() {
  ...
  try {
    ...
    if (!CRYPTO_RACE_ABI) {
      throw new Error("❌ CRYPTO_RACE_ABI not available");
    }
    console.log("✅ CRYPTO_RACE_ABI available");

    // Test ABI structure
    console.log("2. Testing ABI structure...");
    const buyGmTokensFunction = CRYPTO_RACE_ABI.find((item: any) => 
    ...
    const contract = new ethers.Contract(contractAddress, CRYPTO_RACE_ABI, mockSigner as any);
