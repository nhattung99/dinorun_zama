...
export async function fetchRaceOutcomesEtherscan(contract: string, user: string) {
  try {
    const apiKey = process.env.REACT_APP_ETHERSCAN_API_KEY;
    if (!apiKey) throw new Error("ETHERSCAN API key missing");
    const topic0 = ethers.id(RACE_OUTCOME_SIG);
...
export async function fetchUserAggregatesFromEvents(
  ...
  try {
    ...
    for (const log of buyRacesLogs) {
      ...
      if (Number.isFinite(count)) racesBought += count;
    }

    // Daily check-ins (each adds +1 race)
    const checkins = Array.isArray(checkinLogs) ? checkinLogs.length : 0;

    // Races executed
    const racesDone = Array.isArray(raceLogs) ? raceLogs.length : 0;
    ...
    const gmEstimated = Math.max(0, gmPublic + gmFromPrizes - 10 * racesBought);
    const pendingEth = Number(ethers.formatEther(pendingEthWei));
    return { availableRaces, gmEstimated, pendingEth, lastSlot };
  } catch {
    return { availableRaces: 0, gmEstimated: 0, pendingEth: 0, lastSlot: null };
  }
}
