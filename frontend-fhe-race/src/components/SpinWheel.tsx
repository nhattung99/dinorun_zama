...
export const RaceWheel: React.FC<RaceWheelProps> = ({
  isRacening,
  onRaceComplete,
  onRace,
  slots,
  ...
  // ✅ Guard để tránh multiple races
  const [hasSpunThisRound, setHasSpunThisRound] = useState(false);

  // Enhanced racening logic - chỉ chạy khi không có contract transaction
  useEffect(() => {
    if (isRacening && !isAnimating && !hasSpunThisRound) {
      ...
      setTimeout(() => {
        ...
        // ✅ Chỉ gọi onRaceComplete nếu không có contract transaction
  ...
  }, [isRacening, slots, onRaceComplete, isAnimating, hasSpunThisRound, targetSlotIndex]);

  // ✅ Reset guard khi không race nữa
  useEffect(() => {
    if (!isRacening) {
  ...
  return (
    <div className="wheel-container">
      ...
      >
        {isRacening || isAnimating ? "Racening..." : "Race"}
      </button>
    </div>
  );
};
