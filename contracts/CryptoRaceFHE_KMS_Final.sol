...
contract CryptoRaceFHE_KMS_Final is SepoliaConfig {
    ...
    event RaceBoughtWithGm(address indexed user, uint64 count);
    event CheckInCompleted(address indexed user, uint256 timestamp);
    event RaceOutcome(address indexed user, uint8 slot, uint256 prizeWei, uint64 gmDelta);
    ...
    mapping(address => uint256) public raceNonce; // increments per raceLiteCommit
    mapping(address => bytes32) public lastOutcomeCommit; // commit of last race outcome
    ...
    function ensureInit(address user) private {
        ...
        userRaces[user] = FHE.asEuint64(0);
        FHE.allowThis(userRaces[user]);
        FHE.allow(userRaces[user], user);
    ...
    function buyRaceWithGm() external {
        ensureInit(msg.sender);
        ...
        euint64 races = userRaces[msg.sender];
        userRaces[msg.sender] = FHE.select(enough, FHE.add(races, FHE.asEuint64(1)), races);
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);
        // Log error state
        ...
        emit RaceBoughtWithGm(msg.sender, 1);
        _bumpVersion(msg.sender);
    }

    // Batch purchase races with GM in a single tx to avoid multiple prompts
    function buyRaceWithGmBatch(uint64 count) external {
        ensureInit(msg.sender);
        require(count > 0 && count <= 50, "invalid count");
        euint64 gm = userGm[msg.sender];
        uint64 cost = count * 10; // 10 GM per race
        ...
        euint64 races = userRaces[msg.sender];
        userRaces[msg.sender] = FHE.select(enough, FHE.add(races, FHE.asEuint64(count)), races);
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);
        emit RaceBoughtWithGm(msg.sender, count);
        _bumpVersion(msg.sender);
    }
    ...
    function dailyGm() external {
        ...
        euint64 races = userRaces[msg.sender];
        userRaces[msg.sender] = FHE.add(races, FHE.asEuint64(1));
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);
    ...
    function race() external {
        // Optimized race: minimal FHE ops (like raceLite)
        ensureInit(msg.sender);

        // Only operate on races handle to minimize HCU
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);

        euint64 races = userRaces[msg.sender];
        ebool canRace = FHE.gt(races, FHE.asEuint64(0));
        userRaces[msg.sender] = FHE.select(canRace, FHE.sub(races, FHE.asEuint64(1)), races);
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);

        // Maintain encrypted error state
        _lastErrors[msg.sender] = LastErrorRec(FHE.select(canRace, NO_ERROR, ERR_NO_RACES), block.timestamp);
        emit ErrorChanged(msg.sender);
        ...
        emit RaceOutcome(msg.sender, uint8(slot), 0, gmDelta);
        emit RaceCompleted(msg.sender, "Race completed");

        // Create commitment for later settlement
        uint256 n = raceNonce[msg.sender];
    ...
    // Minimal-HCU race: only consume a race and emit outcome. No prize application here.
    function raceLite() external {
        ensureInit(msg.sender);
        // Re-grant only races handle to keep ops minimal
        FHE.allowThis(userRaces[msg.sender]);
        FHE.allow(userRaces[msg.sender], msg.sender);
        ...
        // Lightweight score increment (+100) as part of raceLite to reflect play action
        FHE.allowThis(encryptedScore[msg.sender]);
        FHE.allow(encryptedScore[msg.sender], msg.sender);
        euint64 scLite = encryptedScore[msg.sender];
        encryptedScore[msg.sender] = FHE.add(scLite, FHE.asEuint64(100));
    ...
    // Prize settlement callable after raceLite outcome has been observed off-chain.
    ...
    function getUserRaces(address user) external view returns (euint64) {
        return userRaces[user];
    }
    ...
    function getEncryptedUserBundle(
        address user
    ) external view returns (euint64 races, euint64 gm, euint64 pendingEthWei, euint64 lastSlot, euint64 score) {
        races = userRaces[user];
        gm = userGm[user];
