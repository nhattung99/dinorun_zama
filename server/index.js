...
async function decryptBundleWithRelayer(contract, provider, user, signer) {
  ...
  const enc = [bundle?.races, bundle?.gm, bundle?.pendingEthWei, bundle?.lastSlot, bundle?.score];
  const dec = [];
  for (const ct of enc) {
...
app.get("/api/user/:addr/state", async (req, res) => {
  try {
    ...
    const merged = {
      races: typeof fhe?.races === "number" && fhe.races > 0 ? fhe.races : fromLedger.availableRaces,
      ...
      source: typeof fhe?.races === "number" && fhe.races > 0 ? "fhe" : "api",
      timestamp: Date.now(),
    };
...
app.post("/api/user/:addr/state", async (req, res) => {
  try {
    ...
    const fields = [
      { ct: bundle?.races, sig: sigs.races },
